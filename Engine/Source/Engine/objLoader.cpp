...
ObjLoader::ObjLoader(std::string objFilePath, std::string mtlFilePath) : shader(nullptr)
{
    ...
    for (const auto& part : m_modelParts) {
        GLuint VAO, VBO;
    ...
    std::unique_ptr<Shader> shader = std::make_unique<Shader>("../Engine/Source/Engine/modelShader.vs", "../Engine/Source/Engine/modelShader.fs");
    SetShader(std::move(shader));
}

void ObjLoader::SetShader(std::unique_ptr<Shader> newShader)
{
...
void ObjLoader::RenderModelPart(const ModelPart& part, GLuint vao) {
    glBindVertexArray(vao);

    auto it = m_materials.find(part.materialName);
    ...
    while (fgets(line, sizeof(line), file_ptr.get())) {
        if (strncmp(line, "v ", 2) == 0) {
            ...
            if (currentModelPart == nullptr) {
                m_modelParts.emplace_back();
                currentModelPart = &m_modelParts.back();
